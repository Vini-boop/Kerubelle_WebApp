...
export function StoreProvider({ children }: { children: ReactNode }) {
    ...
    const loadData = useCallback(async () => {
        ...
        setError(null);
        try {
            // Always load products and promotions
            const requests: Promise<any>[] = [
                apiService.getProducts(),
                apiService.getPromotions()
            ];

            // Only load admin-only data if user is admin or staff
            if (isAdmin) {
                requests.push(
            ...
            setPromotions(results[1]);

            if (isAdmin && results.length > 2) {
                setOrders(results[2]);
                setPayments(results[3]);
    ...
    // Load initial data + set up polling for real-time updates (every 30s)
    useEffect(() => {
        if (!authLoading) {
            loadData();
        }
    ...
    // Removed polling interval to prevent Neon data transfer quota drain
    // useEffect(() => {
    ...
    const processPayment = useCallback(async (orderId: string): Promise<{ success: boolean; payment?: Payment }> => {
        ...
        try {
            ...
            // Fire payment notification event
            window.dispatchEvent(new CustomEvent('kerubelle:payment-received', {
                detail: {
                    orderId: order.id,
                    customerName: order.customerName,
                    amount: order.total,
    ...
    const getDashboardSummary = useCallback((): DashboardSummary => {
        ...
        const totalCostOfGoods = paidOrders.reduce((sum, o) =>
            sum + ((o.items || []).reduce((s, i) => s + (i.costPrice || 0) * (i.quantity || 0), 0)), 0
        );
        ...
        return {
            ...
            netProfit: (totalRevenue - totalCostOfGoods - totalExpensesAmount) || 0,
            pendingPayments: pendingPayments || 0,
            totalCostOfGoods: totalCostOfGoods || 0,
            totalExpenses: totalExpensesAmount || 0,
